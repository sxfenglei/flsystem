...
class Page{
    ...
    private $config  = array(
        ...
        'last'   => '尾页',
        'end' => '尾页',
        'theme'  => '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%  %NOW_PAGE%/%TOTAL_PAGE% 页',
    );
    ...
    //select page 拼接
    public function selectPageHtml(){
        $selectPage = '';
        $selectPage = '<select name="sldd" style="width:6rem" onchange="location.href=this.options[this.selectedIndex].value;">';
        $selectPage .= '<option value=".">[跳转]</option>';
        for($i=1;$i<=$this->totalPages;$i++){
            $selectPage .= '<option value="'.__APP__.'/'.MODULE_NAME.'/'.CONTROLLER_NAME.'/'.ACTION_NAME.'/p/'.$i.'">跳到'.$i.'页</option>';
        }
        $selectPage .= '</select>';
        return $selectPage;
    }
    ...
    public function show() {
        ...
        $page_str = str_replace(
            ...
            $this->config['theme']);
        return "<ul class='pagination'>{$page_str}</ul>". $this->selectPageHtml();
    }
}
