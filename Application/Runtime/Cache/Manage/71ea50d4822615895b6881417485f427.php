      ...
      <ul class="sidebar-menu">
        ...
        <?php else: ?>
          ...
          <ul class="treeview-menu">
            <?php if(ACTION_NAME == 'authList'): ?><li class="treeview active">
            <?php else: ?>
              <li class="treeview"><?php endif; ?>
              <a href="<?php echo U('Auth/authList');?>">管理员权限规则</a>
            </li>

            <?php if(ACTION_NAME == 'authGroupList'): ?><li class="treeview active">
            <?php else: ?>
              <li class="treeview"><?php endif; ?>
              <a href="<?php echo U('Auth/authGroupList');?>">管理员角色</a>
            </li>
          </ul>
        </li>
...
<div class="row">
  <div class="col-md-2">
    <a href="<?php echo U('Auth/authGroupAdd');?>" ="" class="btn btn-primary"><i class="fa fa-fw fa-pencil"></i> 新增角色</a>
  </div>
</div><br>
<div class="row">
  <div class="col-md-12">
           ...
           <!--    <form action="<?php echo U('Admin/adminList');?>" method="get">
            ...
            <div class="box-body">
               <table class="table table-bordered">
                  ...
                  <?php if(is_array($authGroupList["list"])): $k = 0; $__LIST__ = $authGroupList["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                    <td><?php echo ($k+$pStr); ?></td>
                    <td><?php echo ($vo["title"]); ?></td>
                    <td><?php echo ($vo["rules_str"]); ?></td>
                    <td>
                      <div class="btn-group">
                          <a href="<?php echo U('Auth/authGroupEdit',array('id'=>$vo['id']));?>" class="btn btn-success">编辑</a>
...
function delFun(id){
  ...
  $.post("<?php echo U('Auth/authGroupDel');?>",{'id':id},function(data){
    // eval("var data="+data);
    if(data.code == 0){
